  ...
  let isActive = pathname === route
    ...
    <div id="Tab" className={cn(
      ...
      <Link href={route} className="flex gap-3 lg:gap-4">
        <Image src={icon} width={30} height={30} alt={label} className="w-5 md:w-6 lg:w-8" />
        <p className="text-lg lg:text-xl font-medium lg:font-semibold">{label}</p>
      </Link>
    </div>
  );
};
...
const SideNav = () => {
    ...
    <div
      id="sideNav"
      className="w-[170px] md:w-[210px] lg:w-[260] xl:w-[300px] h-full bg-background relative pt-5 select-none"
    >
